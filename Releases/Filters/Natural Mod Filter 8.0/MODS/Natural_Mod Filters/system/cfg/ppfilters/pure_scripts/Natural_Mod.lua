...
function update_pure_script(dt)
	...
	cbe_scaler = math.min(math.max(0,last_ae/(4.2*day_compensate(0.1))),1)-- replace with intD or similar so you dont get harsh transition at -3 to -12 sunangle?

	contrast_min = math.max(math.min(__IntD(1,0.93,0.95),0.985),0.975)--outer values are the desired range, inner are wider to give more control
	...
	ac.setPpContrast(night_compensate(math.min(math.lerp(1,contrast_min,cbe_scaler)),contrast_max))-- higher value than actual max used to widen curve
	...
	ac.setPpTonemapFilmicContrast(0.15 + (0.25 * night_compensate(0)))--review
	...
	ac.setGodraysLength(__IntD(5.5,-2,0.4) * PURE__getGodraysModulator())--consider the angle adjustment code
